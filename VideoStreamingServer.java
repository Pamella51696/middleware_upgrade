    ...
    private static class PlayerPageHandler implements HttpHandler {
        ...
        @Override public void handle(HttpExchange ex) throws IOException {
            String html = "<!DOCTYPE html><html lang='en'><head>"
                ...
                + "html, body { height: 100%; width: 100%; background: #0a0a0f; color: #e0e0e0;"
                + "  font-family: 'Segoe UI', sans-serif; overflow: hidden; }"
                + ".container { display: flex; flex-direction: column;"
                + "  width: 100%; height: 100vh; margin: 0; padding: 0; }"
                + "h1 { font-size: 1.1rem; font-weight: 300; letter-spacing: 2px;"
                + "  color: #7ec8e3; text-align: center; flex-shrink: 0;"
                + "  line-height: 1.2; padding: 10px 12px 8px; }"
